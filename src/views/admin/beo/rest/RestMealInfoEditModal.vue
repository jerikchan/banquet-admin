<template>
  <PageWrapper title="余留餐修改" contentBackground @back="goBack">
...
<script lang="ts">
  ...
  import { updateRestMealInfo, getRestMealInfo } from '/@/api/admin/beo';
  ...
  export default defineComponent({
    ...
    setup() {
      ...
      const restMealId = ref(route.params?.id);
      ...
      const [registerFirst, { setFieldsValue: setFirstValues, getFieldsValue: getFirstValues }] =
        useForm({
          labelCol: {
            span: 8,
          },
      ...
      const [
        registerRestFood,
        { setFieldsValue: setRestFoodsValues, getFieldsValue: getRestFoodsValues },
      ] = useForm({
        labelCol: {
          span: 2,
        },
      ...
      async function submitAll() {
        try {
          ...
          // await updateSiYiChatForm(submitValues);
          ...
          submitValues.id = restMealId.value;
          await updateRestMealInfo(submitValues);
      ...
      async function handleData(id: string) {
        res = await getRestMealInfo({ id: id });
        setFieldsValue({
          ...res,
        });
        setFirstValues({
