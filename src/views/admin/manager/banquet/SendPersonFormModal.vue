<template>
  <PageWrapper title="人员派单登记记录修改" contentBackground @back="goBack">
...
<script lang="ts">
  ...
  import { sendPersonFormSchema } from './manager.data';
  import { getSendPersonFormDetail, updateSendPersonForm } from '/@/api/admin/manager';
    ...
    setup() {
      ...
      async function submitAll() {
        try {
          ...
          await updateSendPersonForm(submitValues);
      ...
      async function handleData(id: string) {
        res = await getSendPersonFormDetail({ id: id });
        setFieldsValue({
          ...res,
        });
      }
